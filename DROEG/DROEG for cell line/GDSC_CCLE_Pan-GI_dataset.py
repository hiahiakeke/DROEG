...
def get_data():
    path = os.getcwd()
    cluster_data = pd.read_csv(r"{}\drug_panfull2.csv".format(path),
                               index_col=0)
    cluster_data = cluster_data
    cluster_data.fillna(cluster_data.mean(),inplace=True)
    regression_data = pd.read_csv(r"{}\drug_panfull2.csv".format(path),
                                  index_col=0)
